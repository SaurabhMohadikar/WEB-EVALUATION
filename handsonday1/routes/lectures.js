const dbb=require('../makedatabase').Lecture
...
route.route("/:id").get((req,res)=>{
    ...
    dbb.findOne({where:{ lectureid :req.params.id}}).
    then(result=>{
        if(result){
            res.json(result);
        }
...
.put((req,res)=>{
    ...
    dbb.update(req.body,{where :{lectureid: req.params.id}})
    .then(result=>{dbb.findOne({where:{lectureid:req.params.id}}).then(ress=>{res.send(ress);console.log(ress)})})
